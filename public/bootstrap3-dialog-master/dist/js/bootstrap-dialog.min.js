(function(b,c){if(typeof module!=="undefined"&&module.exports){var d=(typeof process!=="undefined");var a=d&&("electron" in process.versions);if(a){b.BootstrapDialog=c(b.jQuery);
}else{module.exports=c(require("jquery"),require("bootstrap"));}}else{if(typeof define==="function"&&define.amd){define("bootstrap-dialog",["jquery","bootstrap"],function(e){return c(e);
});}else{b.BootstrapDialog=c(b.jQuery);}}}(this,function(d){var b=d.fn.modal.Constructor;var c=function(f,e){b.call(this,f,e);};c.getModalVersion=function(){var e=null;
...
}else{e="v3.3.4";}}}return e;};c.ORIGINAL_BODY_PADDING=parseInt((d("body").css("padding-right")||0),10);c.METHODS_TO_OVERRIDE={};c.METHODS_TO_OVERRIDE["v3.1"]={};
c.METHODS_TO_OVERRIDE["v3.2"]={hide:function(g){if(g){g.preventDefault();}g=d.Event("hide.bs.modal");this.$element.trigger(g);if(!this.isShown||g.isDefaultPrevented()){return;
}this.isShown=false;var f=this.getGlobalOpenedDialogs();if(f.length===0){this.$body.removeClass("modal-open");}this.resetScrollbar();this.escape();d(document).off("focusin.bs.modal");
this.$element.removeClass("in").attr("aria-hidden",true).off("click.dismiss.bs.modal");d.support.transition&&this.$element.hasClass("fade")?this.$element.one("bsTransitionEnd",d.proxy(this.hideModal,this)).emulateTransitionEnd(300):this.hideModal();
...
a.DEFAULT_TEXTS[a.TYPE_INFO]="Information";a.DEFAULT_TEXTS[a.TYPE_PRIMARY]="Information";a.DEFAULT_TEXTS[a.TYPE_SUCCESS]="Success";a.DEFAULT_TEXTS[a.TYPE_WARNING]="Warning";
a.DEFAULT_TEXTS[a.TYPE_DANGER]="Danger";a.DEFAULT_TEXTS.OK="Si";a.DEFAULT_TEXTS.CANCEL="No";a.DEFAULT_TEXTS.CONFIRM="Confirmación";a.SIZE_NORMAL="size-normal";
...
a.BUTTON_SIZES[a.SIZE_WIDE]="";a.BUTTON_SIZES[a.SIZE_LARGE]="btn-lg";a.ICON_SPINNER="glyphicon glyphicon-asterisk";a.defaultOptions={type:a.TYPE_PRIMARY,size:a.SIZE_NORMAL,cssClass:"",title:null,message:null,nl2br:true,closable:true,closeByBackdrop:true,closeByKeyboard:true,spinicon:a.ICON_SPINNER,autodestroy:true,draggable:false,animate:true,description:"",tabindex:-1};
...
});};a.closeAll=function(){d.each(a.dialogs,function(f,e){e.close();});};a.getDialog=function(f){var e=null;if(typeof a.dialogs[f]!=="undefined"){e=a.dialogs[f];
}return e;};a.setDialog=function(e){a.dialogs[e.getId()]=e;return e;};a.addDialog=function(e){return a.setDialog(e);};a.moveFocus=function(){var e=null;
d.each(a.dialogs,function(g,f){e=f;});if(e!==null&&e.isRealized()){e.getModal().focus();}};a.METHODS_TO_OVERRIDE={};a.METHODS_TO_OVERRIDE["v3.1"]={handleModalBackdropEvent:function(){this.getModal().on("click",{dialog:this},function(e){e.target===this&&e.data.dialog.isClosable()&&e.data.dialog.canCloseByBackdrop()&&e.data.dialog.close();
...
if(typeof e.enabled!=="undefined"){f.toggleEnable(e.enabled);}return f;},enhanceButton:function(e){e.dialog=this;e.toggleEnable=function(f){var g=this;
if(typeof f!=="undefined"){g.prop("disabled",!f).toggleClass("disabled",!f);}else{g.prop("disabled",!g.prop("disabled"));}return g;};e.enable=function(){var f=this;
f.toggleEnable(true);return f;};e.disable=function(){var f=this;f.toggleEnable(false);return f;};e.toggleSpin=function(i){var h=this;var g=h.dialog;var f=h.find("."+g.getNamespace("button-icon"));
if(typeof i==="undefined"){i=!(e.find(".icon-spin").length>0);}if(i){f.hide();e.prepend(g.createButtonIcon(g.getSpinicon()).addClass("icon-spin"));}else{f.show();
e.find(".icon-spin").remove();}return h;};e.spin=function(){var f=this;f.toggleSpin(true);return f;};e.stopSpin=function(){var f=this;f.toggleSpin(false);
return f;};return this;},createButtonIcon:function(f){var e=d("<span></span>");e.addClass(this.getNamespace("button-icon")).addClass(f);return e;},enableButtons:function(e){d.each(this.indexedButtons,function(g,f){f.toggleEnable(e);
...
if(e.isAutodestroy()){e.setRealized(false);delete a.dialogs[e.getId()];d(this).remove();}a.moveFocus();});this.handleModalBackdropEvent();this.getModal().on("keyup",{dialog:this},function(e){e.which===27&&e.data.dialog.isClosable()&&e.data.dialog.canCloseByKeyboard()&&e.data.dialog.close();
...
this.updateTabindex();return this;},open:function(){!this.isRealized()&&this.realize();this.getModal().modal("show");return this;},close:function(){!this.isRealized()&&this.realize();
this.getModal().modal("hide");return this;}};a.prototype=d.extend(a.prototype,a.METHODS_TO_OVERRIDE[c.getModalVersion()]);a.newGuid=function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(g){var f=Math.random()*16|0,e=g==="x"?f:(f&3|8);
